...
def get_menu_items():
    """
    Summary or Description of the Function 
    returns server.html, displays all menu items, ingredients, price, and gives user a place to enter the quantity they want to order
    ...
    7) loop through all smoothies
        if an ingredient is used in a certain smoothie then display it on server.html
    ...
    return render_template('server.html', data = newData)
...
def get_ingredients():
    ...
    """
    Summary or Description of the Function 
    creates a hashmap that has each inventory item and which all smootheis use that item
...
def place_order_server(data):
    """
    Summary or Description of the Function 
    function to loop through the user input on the server page and push it into the temp order table
